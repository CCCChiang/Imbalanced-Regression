...
class DataClean:
    def choose_float(self, data):
        return data.loc[:, (data.dtypes != object)]

    def drop_unit(self, data):
        __mask = data.apply(lambda x: x.nunique() == 1)
        __mask = ~np.array(__mask)

        return data.loc[:, __mask]
    
    def drop_No(self, data):
        __mask = list(map(lambda x: x.find("No") != -1, data.columns))
        __mask = ~np.array(__mask)

        return data.loc[:, __mask]

    def drop_miss(self, data, ratio):
        __mask = data.apply(lambda x: x.isnull().sum()/len(x) >= ratio )
        __mask = ~np.array(__mask)

        return data.loc[:, __mask]

    def drop_dup(self, data, keep='first'):

        return data.drop_duplicates(inplace=False, keep=keep)

    def drop_na(self, data, axis=0, how="any"):

        return data.dropna(axis=axis, how=how)
    
    def drop_JND_greater_digit(self, data, jnd, col):
        __mask = data.loc[:, col] > jnd
        __mask = ~np.array(__mask)
